...
def test_calculate_global_rating(test_client, questions_without_ratings, survey_data_for_one_question_with_big_score_diff, create_user_in_db):
    ...
    #user = User.query.filter_by(id = survey_data_for_one_question_with_big_score_diff['user_id']).first()
    #
    # bad_response = test_client.post('/admin/api/rating' ,json = {'question_id' : question_id}) #to fake app context
    #
    # login_user_for_test(test_client,user)
    #
    # response = test_client.post('/admin/api/rating' ,json = {'question_id' : question_id})
    ...
    assert response.status_code == 200
    assert response.get_json() == {'message': 'No choices found'}
